...
class SubgoalCoordinateFilter():
    def __init__(self, args):
        ...
        self.map_save_dir = os.path.join(args.data_path, "vlmaps")
    ...
    def prepare_maps_and_map_params(self):
        ...
        self.obstacles = torch.tensor(self.load_map(self.obstacles_save_path), device=self.device)
        self.grid = torch.tensor(self.load_map(self.grid_save_path), device=self.device)

        if self.do_crop:
            x_indices, y_indices = torch.where(self.obstacles == 0)
            self.xmin = torch.min(x_indices).item()
            self.xmax = torch.max(x_indices).item()
            self.ymin = torch.min(y_indices).item()
            self.ymax = torch.max(y_indices).item()
    ...
    def get_text_feats(self, in_text, clip_model, clip_feat_dim, batch_size=64):
        ...
        text_id = 0
        text_feats = torch.zeros((len(in_text), clip_feat_dim), dtype=torch.float32, device=self.device)
    ...
    def find_navigable_direction_with_raycast(self, contour_point, resolution=15, direction=None, reverse=False):
        ...
        contour_point = torch.tensor(contour_point, device=self.device)  # Convert to tensor    
        angles = torch.tensor(self.get_direction_angles_in_deg(direction, resolution), device=self.device)
    ...
    def get_pixel_label_and_confidence(self, lang):
        ...
        map_feats = self.grid.view(-1, self.grid.shape[-1])
        ...
        return pixel_label.to(self.device), pixel_confidence.to(self.device)
