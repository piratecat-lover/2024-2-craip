...
class SCFArgs():
    def __init__(self):
        ...
        self.device = torch.device(rospy.get_param(rospy.get_name() + '/device'))  # Use torch.device
...
class SCFWrapper():
    def __init__(self, args):
        ...
        # New subscriber to detected landmarks from YOLO
        self.detected_landmark_sub = rospy.Subscriber("/detected_landmark", String, self.detected_landmark_callback)
    ...
    def detected_landmark_callback(self, msg):
        landmark = msg.data
        rospy.loginfo(f"Received detected landmark: {landmark}")
        
        # Call go_to method to set subgoal for this landmark
        self.scf.set_robot_pose(self.robot_pixel_pose)
        self.go_to(landmark)

        # Refresh subgoal list and publish
        self.subgoal_list = []
        self._infill_subgoal_list()
        self.publish_goal()
